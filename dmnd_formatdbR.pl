...
#PROGRAM NAME
my $formatdbName = programName('diamond makedb');
...
OPTIONS
   * Use the usual formatdb parameters (for instance \"formatdbR.pl --in inputDatabase --db outDatabaseName\")
...
foreach my $field (0..$#ARGV){
  #the inputs
  if (($ARGV[$field] eq '--in') and ($spyInput == 1)){
    $spyInput = 2;
     next;
  }
  if ($spyInput == 2){
...
if (defined $commandLine){
  $command = "$formatdbName --in $translatedFile "." $commandLine";
}
else{
  $command = "$formatdbName --in $translatedFile ";
}
